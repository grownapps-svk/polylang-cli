...
class TermCommand extends BaseCommand
{
     ...
     *
     * [<language-code>]
     * : Language code (slug), or comma-separated list of language codes, to duplicate the term to. 
    ...
    private function duplicate_term($taxonomy, $term, $slug)
    {
        ...
        if ($slug === $term_language) {
            ...
            if (empty($duplicate)) {
                ...
                $this->api->save_term_translations(
                    array_unique(
                        array_merge(
                            array($term_language => $term_id, $slug => $duplicate['term_id']),
                            $this->api->get_term_translations($term_id)
                        )
                    )
                );
